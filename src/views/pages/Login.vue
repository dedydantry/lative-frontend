...
export default{
  ...
  methods:{
    ...
    async postLogin(){
      try {
        this.$vs.loading()
        let post = await this.$axios.post('/sign',{
          email:this.email,
          password:this.password
        })
        this.$vs.loading.close()
        if(post.status){
          this.setUser(post.data.user)
          this.setToken(post.data.token)
        ...
        if(post.data == 'Account is not activated') return setTimeout(() => {
          this.$router.push(`/auth-info?email=${this.email}`)
        }, 2000)
      } catch (error) {
        this.$vs.loading.close()
        this.$vs.notify({
